...
def _process_single_shape(shape_id: str, df, matcher: OSRMMapMatcher, cleaner: ShapeCleaner, logger: logging.Logger, max_stub_meters: float = 40.0, enable_stub_filter: bool = True):
    thread_name = current_thread().name
    logger.debug(f"Starting processing for shape {shape_id} with {len(df)} points on thread {thread_name}")
    
    results = []
    try:
        # 0. Pre-matching perpendicular stub filter (Enabled by default)
...
def main():
    ...
    process_gtfs_shapes(
        ...
        enable_stub_filter=not args.no_stub_filter
    )
